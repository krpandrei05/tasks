...
class TaskServiceTest {
    ...
    void updateTaskContent_updatesNameAndReturnsDto_whenTaskExists() {
        ...
        TaskDTO result = taskService.updateTaskContent(100L, "Write handover v2");
    ...
    @Test
    void updateTaskContent_throwsTaskNotFoundException_whenTaskDoesNotExist() {
        when(taskRepository.findById(999L)).thenReturn(Optional.empty());
    ...
    @Test
    void getTasksByStatus_returnsOnlyAccessibleTasks() {
        ...
        when(taskRepository.findByStatusType_StatusName("Pending"))
    ...
    @Test
    void getTasksCount_returnsCountFromRepository() {
        when(taskRepository.count()).thenReturn(5L);

        int result = taskService.getTasksCount();

        assertEquals(5, result);
    }

    @Test
    void getOverdueTasks_returnsOnlyAccessibleTasks() {
        Task accessibleTask = existingTask;
        Task inaccessibleTask = Task.builder()
                .taskId(200L)
                .taskName("Someone else's overdue task")
                .statusType(pendingStatus)
                .user(User.builder().userId(2L).email("other@example.com").build())
                .dueDate(existingTaskDto.getDueDate())
                .build();

        when(taskRepository.findOverdueTasksExcludingDone(any(LocalDateTime.class)))
                .thenReturn(List.of(accessibleTask, inaccessibleTask));
        when(permissionChecker.canAccessTask(accessibleTask)).thenReturn(true);
        when(permissionChecker.canAccessTask(inaccessibleTask)).thenReturn(false);
        when(taskMapper.toDto(accessibleTask)).thenReturn(existingTaskDto);
